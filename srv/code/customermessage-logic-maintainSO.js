...
/**
 * 
 * @On(event = { "Action2" }, entity = "minhwanhwang_22_STUDENTH22Srv.CustomerMessage")
...
module.exports = async function (request) {
	...
	try {
		...
		customerMessage = await SELECT.one.from('minhwanhwang_22_STUDENTH22.CustomerMessage').where({ ID });
		if (!customerMessage) {
	...
	const { titleEnglish, fullMessageEnglish, suggestedResponseEnglish, a_ServiceOrder_ServiceOrder: attachedSOId } = customerMessage;
	...
	if (attachedSOId) {
		...
		try {
			...
			await UPDATE('minhwanhwang_22_STUDENTH22.CustomerMessage')
