...
/**
 * 
 * @After(event = { "CREATE","UPDATE" }, entity = "minhwanhwang_22_STUDENTH22Srv.ProductFAQ")
...
module.exports = async function (results, request) {
	...
	try {
		...
		productFAQ = await SELECT.one('minhwanhwang_22_STUDENTH22.ProductFAQ').where({ ID: productFAQID }).forUpdate();
		if (!productFAQ) {
	...
	try {
		...
		await UPDATE('minhwanhwang_22_STUDENTH22.ProductFAQ').set({ embedding: embedding }).where({ ID });
